...
from spiral_nemo.utils.env_var_parsing import get_envint


def is_global_rank_zero(trainer=None):
    """ Helper function to determine if the current process is global_rank 0 (the main process)
    """
    if trainer is not None and trainer.use_horovod:
        return trainer.distributed_sampler_kwargs['rank'] == 0
