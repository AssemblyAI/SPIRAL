...
#            - fix global rank check when use horovod
#            - resume checkpoint if exists as default
#            - increase precision of eval metric in checkpoint name
...
from spiral_nemo.constants import NEMO_ENV_VARNAME_VERSION
from spiral_nemo.utils import logging
from spiral_nemo.utils.exceptions import NeMoBaseException
from spiral_nemo.utils.get_rank import is_global_rank_zero
from spiral_nemo.utils.lightning_logger_patch import add_filehandlers_to_pl_logger
...
class ExpManagerConfig:
    ...
    resume_if_exists: Optional[bool] = True
    resume_past_end: Optional[bool] = False
    resume_ignore_no_checkpoint: Optional[bool] = True
    # Logging parameters
...
def exp_manager(trainer: 'pytorch_lightning.Trainer', cfg: Optional[Union[DictConfig, Dict]] = None) -> Path:
    ...
    if is_global_rank_zero(trainer):
...
def check_explicit_log_dir(
    ...
    if is_global_rank_zero(trainer) and Path(explicit_log_dir).exists():
...
def configure_checkpointing(
    ...
    if params.filepath is None:
        params.filepath = Path(log_dir / 'checkpoints' / f'-{{{params.monitor}:.4f}}-{{epoch}}')
