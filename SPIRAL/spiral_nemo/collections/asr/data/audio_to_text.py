...
from spiral_nemo.collections.asr.parts import collections, parsers
from spiral_nemo.collections.asr.parts.features import WaveformFeaturizer
from spiral_nemo.core.classes import Dataset, IterableDataset
...
class _AudioTextDataset(Dataset):
    ...
    Args:
        ...
        augmentor (spiral_nemo.collections.asr.parts.perturb.AudioAugmentor): An AudioAugmentor object used to augment loaded
            audio
    ...
    def __getitem__(self, index):
        ...
        if self.load_audio:
            ...
            audio_fp = sample.audio_file
            if self.data_dir:
                audio_fp = os.path.join(self.data_dir, audio_fp)
            features = self.featurizer.process(
                audio_fp, offset=offset, duration=sample.duration, trim=self.trim, orig_sr=sample.orig_sr
            )
...
class AudioDataset(Dataset):
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        if self.return_both:
            return {
                ...
                'a_sig_length': NeuralType(tuple('B'), LengthsType()),
                'p_audio_signal': NeuralType(
                    ('B', 'T'),
                    AudioSignal(freq=self._sample_rate)  # TODO: self._sample_rate is not defined anywhere
                    if self is not None and hasattr(self, '_sample_rate')
                    else AudioSignal(),
                ),
                'p_a_sig_length': NeuralType(tuple('B'), LengthsType()),
            }
        else:
            return {
                'audio_signal': NeuralType(
    ...
    def __getitem__(self, index):
        ...
        if self.data_dir:
            audio_fp = os.path.join(self.data_dir, audio_fp)
        features = self.featurizer.process(
            audio_fp, offset=offset, duration=sample.duration, trim=self.trim, orig_sr=sample.orig_sr, crop_size=self.crop_size
        )
        ...
        if not self.return_both:
            ...
            clean_feat, perturbed_feat = features
            fl = torch.tensor(clean_feat.shape[0]).long()
            pfl = torch.tensor(perturbed_feat.shape[0]).long()
            return clean_feat, fl, perturbed_feat, pfl
...
def _speech_only_collate_fn(batch):
    """collate batch of audio sig, audio len, tokens, tokens len
    Args:
        batch (Optional[FloatTensor], Optional[LongTensor], LongTensor,
               LongTensor):  A tuple of tuples of signal, signal lengths,
    ...
    assert audio_lengths[0] is not None
    max_audio_len = max(audio_lengths).item()
...
def _both_speech_only_collate_fn(batch):
    """collate batch of audio sig, audio len, tokens, tokens len
    Args:
        batch (Optional[FloatTensor], Optional[LongTensor], LongTensor,
               LongTensor):  A tuple of tuples of signal, signal lengths,
               ...
               assumes the signals are 1d torch tensors (i.e. mono audio).
    """
    _, audio_lengths, _, perturbed_lengths = zip(*batch)
    assert audio_lengths[0] is not None
    max_audio_len = max(audio_lengths).item()
    max_perturbed_len = max(perturbed_lengths).item()

    audio_signal = []
    perturbed_signal = []
    for sig, sig_len, p_sig, p_sig_len in batch:
        sig_len = sig_len.item()
        if sig_len < max_audio_len:
            pad = (0, max_audio_len - sig_len)
            sig = torch.nn.functional.pad(sig, pad)
        ...
        p_sig_len = p_sig_len.item()
        if p_sig_len < max_perturbed_len:
            pad = (0, max_perturbed_len - p_sig_len)
            p_sig = torch.nn.functional.pad(p_sig, pad)
        perturbed_signal.append(p_sig)

    audio_signal = torch.stack(audio_signal)
    audio_lengths = torch.stack(audio_lengths)

    perturbed_signal = torch.stack(perturbed_signal)
    perturbed_lengths = torch.stack(perturbed_lengths)

    return audio_signal, audio_lengths, perturbed_signal, perturbed_lengths
...
class AudioToCharDataset(_AudioTextDataset):
    ...
    Args:
        ...
        sample_rate (int): Sample rate to resample loaded audio to
        int_values (bool): If true, load samples as 32-bit integers. Defauts to False.
        augmentor (spiral_nemo.collections.asr.parts.perturb.AudioAugmentor): An AudioAugmentor
    ...
    def __init__(
        ...
        parser = parsers.make_parser(
            labels=labels, name=parser, unk_id=unk_index, blank_id=blank_index, do_normalize=normalize, add_end_space=parser_add_end_space
        )
...
class AudioToBPEDataset(_AudioTextDataset):
    ...
    def __init__(
        ...
        data_dir: str = '',
        dup_factor: int = 1,
        sampling_nbest_size: Optional[int] = None,
        ...
        class TokenizerWrapper:
            ...
            def __call__(self, text):
                t = self._tokenizer.text_to_ids(text, sampling_nbest_size, sampling_alpha)
                return t
        ...
        super().__init__(
            ...
            data_dir=data_dir,
            dup_factor=dup_factor,
            parse_online=sampling_nbest_size is not None
        )
