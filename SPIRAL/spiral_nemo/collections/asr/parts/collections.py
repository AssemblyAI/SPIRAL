...
#            - allow ignore text in speech text dataset
...
from spiral_nemo.collections.asr.parts import manifest, parsers
...
class AudioText(_Collection):
    ...
    def __init__(
        ...
        if dup_factor > 1:
            assert not index_by_file_id
            data = [d_i for d_i in data for _ in range(dup_factor)]
            logging.info("Dataset duplicated %d times", dup_factor)

        super().__init__(data)

    def get_item_online(self, i):
        assert self.parse_online
        data = super().__getitem__(i)
        text_tokens = self.parser(data.text_raw)
        return data._replace(text_tokens=text_tokens)

    def __getitem__(self, i):
        if self.parse_online:
            return self.get_item_online(i)
        else:
