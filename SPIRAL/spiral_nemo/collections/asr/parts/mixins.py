...
#            - support subword tokenizer for convtt
#            - open bpe vocab with utf-8
#            - make predending unk to vocab optional
#            - make register_artifact optional in BPEMixin
...
from spiral_nemo.collections.common import tokenizers
...
class ASRBPEMixin(ABC):
    ...
    def _setup_tokenizer(self, tokenizer_cfg: DictConfig, register_artifact=True):
        ...
        self.tokenizer_file = self.tokenizer_cfg.pop('file')
        ...
        if self.tokenizer_type == 'bpe':
            ...
            tokenizer_file = self.tokenizer_file + '.model' if self.tokenizer_file else 'tokenizer.model'
            model_path = os.path.join(self.tokenizer_dir, tokenizer_file)
            ...
            vocab_file = self.tokenizer_file + '.vocab' if self.tokenizer_file else 'vocab.txt'
            vocab_path = os.path.join(self.tokenizer_dir, vocab_file)
            if register_artifact:
                vocab_path = self.register_artifact('tokenizer.vocab_path', vocab_path)
            self.vocab_path = vocab_path

            if self.tokenizer_cfg.get('prepend_unk_to_vocab', True):
                vocabulary = {0: '<unk>'}
            else:
                vocabulary = {}
