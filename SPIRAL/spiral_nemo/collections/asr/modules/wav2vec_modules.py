...
#            - allow getting mask_positions
#            - log temperature and quantize_ppl
#            - optimize quantization memory usage when only one group
...
class GumbelVectorQuantizer(NeuralModule):
    ...
    def forward(self, x):
        ...
        prob_ppl = torch.exp(-torch.sum(avg_probs * torch.log(avg_probs + 1e-7), dim=-1)).sum()
        prob_ppl_loss = (num_vars - prob_ppl) / num_vars
        ...
        if self.groups == 1:
            x = torch.matmul(x, vars.squeeze(0))
        else:
        ...
        return x, prob_ppl_loss, cur_codebook_temp, prob_ppl
...
def compute_mask_indices(
    ...
    mask_num = [len(m) for m in mask_idcs]
    min_len = min(mask_num)
    for i, mask_idc in enumerate(mask_idcs):
        if shrink_to_batch_min and len(mask_idc) > min_len:
            mask_idc = np.random.choice(mask_idc, min_len, replace=False)
        mask[i, mask_idc] = True
        if mask_positions is not None:
            mask_positions.append(mask_idc)
