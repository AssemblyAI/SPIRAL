...
#            - disable computing training wer, print eval wer of first three batches
#            - support setting blank id for CTC models
#            - support running CTC bpe models
#            - return test hypotheses and references in ctc_finetune
...
from spiral_nemo.collections.asr.parts.rnnt_utils import Hypothesis
...
class WER(Metric):
    ...
    def __init__(
        ...
        *,
        ...
        self.strip_end_space = strip_end_space
        if self.strip_end_space:
            print('INFO: WER strip_end_space')
    ...
    def update(
        ...
        if self.strip_end_space:
            # in preprocess, there may be space added at the end
            references = [ref_i.rstrip(' ') for ref_i in references]
            hypotheses = [hyp_i.rstrip(' ') for hyp_i in hypotheses]
